...
export default function Hero({ onMapReady }: HeroProps) {
  ...
  const handleScrollDown = () => {
    const nextSection = document.getElementById('proxima-sessao');
    if (nextSection) {
  ...
  return (
    <section className={`relative bg-primary-dark min-h-screen flex items-center ${outfit.className} overflow-hidden`}>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10 w-full">
        ...
        {/* Seta Scroll (COM ONCLICK AGORA) */}
        <motion.div 
           initial={{ opacity: 0, y: -20 }}
           animate={{ opacity: 1, y: 0 }}
           transition={{ delay: 2, duration: 1 }}
           onClick={handleScrollDown} // <--- ADICIONADO AQUI
           className="absolute bottom-8 left-1/2 -translate-x-1/2 text-orange-500/50 animate-bounce cursor-pointer hover:text-orange-500 transition-colors z-20 p-2"
        >
          <ChevronDown size={32} />
        </motion.div>
      </div>
